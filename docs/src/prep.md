...
# Julia onofficial packages from repository
Pkg.add(PackageSpec(url="https://github.com/EMCMS/ACS.jl"))
```
The second package that is beeing installed contains all relevant functions and information for the Advanced Chemometrics and Statistics course.
...
#### While
In case there is not a clear range over which the iteration must take place, while loops could be used, which continue untill the condition is not true is reached. The exmample below prints and adds 1 to the variable *i* untill *i* = 9, for which the *i* <= 8 yields false.
```julia
i = 1
while i <= 8
    println(i)
...
For example, using an if-else statment, only the even numbers can be printen by adding a if statement in the for loop that evaluates the remained by dividing a value by 2. 
```julia
for i = 1:12
    if rem(i,2) .== 0   
        println(i)
...
#### scope
One thing to consider when building functions is the scope of the variables. A variale can have a local or a global scope. Variables with a local scope can, for example, only be seen within a function, while global variables can be seen through out the "file", but not within functions. That is why these variables need to be passed down to a function, for example the variable numbers in the case below. Same as to why we need to return *res* to be able to used is outside the function in the global scope, which is saved in vairable *out*. When running the code below you will norice that println(res) will actually return an error, since to the global scope this variable is not defined.
...
### Loading/saving data
Loding and saving of DataFrames can be done through the following lines from .csv files.
```julia
#loading data from csv files
data =  CSV.load("path2data", DataFrame)

#savind data to a csv file
